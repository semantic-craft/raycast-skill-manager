...
> Verified against each provider's docs (2026); these are the **Token Plan** endpoints where a provider offers one. For Qwen text models, use Token Plan here; keep DashScope reserved for Qwen-TTS projects. MiniMax international legacy is `https://api.minimax.io/anthropic`; Xiaomi MiMo is per-cluster (China `token-plan-cn`, Singapore `token-plan-sgp`, Europe `token-plan-ams`), authoritative on its subscription page. Set the model id (e.g. `qwen3.6-plus`, `qwen3.6-flash`, `MiniMax-M2.7-highspeed`, `deepseek-v4-flash`, `mimo-v2.5`) in **Custom Model**.
